...
from dataloader.kitti_rr_loader import KITTINMPairDataset
...
def evaluate(config, data_loader, method):
    ...
    for i in range(len(data_loader)):
        ...
        t0 = data_dict['extra_packages'][0]['t0']
        t1 = data_dict['extra_packages'][0]['t1']
        os.makedirs(os.path.join(savedir, f'{drive:02d}'), exist_ok=True)
        savepath = os.path.join(savedir, f'{drive:02d}/{t0}_{t1}.npz')
        if os.path.exist(savepath):
            if (np.load(savepath)['trans']).shape[0]:
                continue
        ...
        np.savez(savepath, trans=T_pred, time=reg_time)
...
if __name__ == '__main__':
    ...
    now = datetime.now()
    logging.basicConfig(filename=f'test_kitti_{now.strftime("%m_%d_%H_%M")}.log',format='%(asctime)s %(message)s',
